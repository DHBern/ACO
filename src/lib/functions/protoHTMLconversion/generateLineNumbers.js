...
export function generateLineNumbers(text) {
	...
	const numbersText = linesText.replace(/<a data-line='(\d+)'><\/a>/g, (match, lineNumber) => {
		return `<a class="line-number" href="?${page.url.searchParams.toString()}&line=${lineNumber}" line="${lineNumber}">${!(lineNumber % 5) ? lineNumber : "<span class='lineNumBuffer'>__</span>"}</a>`;
	});
